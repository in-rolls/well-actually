...
`make land-measurement` reads the local `LAND_REPO` (default `../land`) without modifying it, scans all raw account geography, and computes caste ownership bounds and sensitivity summaries. It needs `arrow`, `data.table`, `readr`, `haven`, `bit64`, `tidyselect`, and `digest`, already recorded in the lockfile. Account locations and village aggregates remain local in Git-ignored Parquet files; public outputs contain aggregate summaries. The geographic audit uses exact identifiers and asserts unique account locations and unambiguous village keys. No new name inference or survey-to-title linkage is performed.

`make land-prefix` first runs the direct-geography measurement target, then performs an exploratory account-prefix linkage check and repeats the ownership analysis under `output/land_prefix/`. The prefix is ten characters, reference groups require at least 20 directly located accounts, and the 20% account holdout uses seed 113780. Prefix length was selected after inspecting the full geography file, so the holdout is an internal diagnostic rather than independent validation of inferred locations. The extended local Parquet file records whether each location was directly observed or inferred; inferred locations do not replace the primary analysis.

Land validation on 2026-09-07: `make land-measurement land-prefix` completed from the raw inputs, and `make test lint` passed all 188 assertions with no lints. Tests cover unknown-acreage bounds, largest-group versus majority distinctions, conflicting account locations, cutoff nesting, invariant acreage after coding changes, and conservation of land and accounts under the prefix extension. Local documentation links were checked.

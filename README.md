...
[Completed IHDS replication](ms/ihds-replication.md): the main later-survey crop-value comparison is negative, with wide uncertainty; changing owned acreage to cultivated crop-season acreage changes the direction. It neither reproduces a stable positive gap nor decisively rules out the 45% benchmark. Run `make ihds-replication` with the local IHDS archive. [Completed land-record measurement checks](ms/external-data-dictionary.md#completed-land-record-measurement-checks) test ownership cutoffs, unknown caste, land definitions and account coverage. The combined land-record/IHDS outcome analysis still needs a village crosswalk.

...
precision <- read.csv("output/precision_ladder.csv")
precision_boot <- read.csv("output/precision_wild_bootstrap.csv")
precision_cells <- read.csv("output/precision_cells.csv")
ihds_precision <- read.csv("output/ihds_inference_ladder.csv")
precision_row <- function(method, contrast = "buyer_gap", table = precision) {
  row <- table[table$method == method & table$contrast == contrast, ]
  stopifnot(nrow(row) == 1L)
  row
}
original_buyer <- precision_row("village_CR1")
corrected_buyer <- precision_row("village_CR2")
bootstrap_buyer <- precision_row("fnw11", table = precision_boot)
ihds_buyer <- precision_row("CR2", table = ihds_precision)
...
All `r nrow(comparison)` displayed coefficient/standard-error pairs checked agree within 0.1 rupee.
The estimated gap for lower-caste water buyers without pumps is **₹`r num(original_buyer$estimate, 0)`
more crop sales per owned acre per year** in lower-caste-dominated villages. The published inference
method gives a 95% interval of **₹`r num(original_buyer$lower, 0)`–₹`r num(original_buyer$upper, 0)`**.
A correction for limited and uneven village information widens it to
**₹`r num(corrected_buyer$lower, 0)`–₹`r num(corrected_buyer$upper, 0)`**. That supports a positive
...
| Full village gap for nonowner water buyers, corrected interval | ₹`r num(corrected_buyer$estimate, 0)` [₹`r num(corrected_buyer$lower, 0)`, ₹`r num(corrected_buyer$upper, 0)`] | [Precision audit](output/precision_ladder.csv) |
...
under the stated method. The buyer interaction alone is ₹`r num(entry("T4_2", "dlbuywater", "estimate"), 0)`
(SE ₹`r num(entry("T4_2", "dlbuywater", "se"), 0)`); the actual village gap for nonowner buyers also
includes the village coefficient. That full contrast is ₹`r num(original_buyer$estimate, 0)`
(SE ₹`r num(original_buyer$std_error, 0)`), using both variances and their covariance. The larger
...
**`r num(corrected_buyer$df, 0)` degrees of freedom** for Anderson's buyer gap. The corresponding
IHDS calculation has **`r num(ihds_buyer$df, 1)`**, because its relevant comparison cells are much
sparser. These are contrast-specific calibrations of uncertainty, not counts of villages. Anderson
has nonowner buyers in `r precision_cells$villages[precision_cells$dominance == 0 & precision_cells$water_status == "buyer"]`
upper-dominated and `r precision_cells$villages[precision_cells$dominance == 1 & precision_cells$water_status == "buyer"]`
...
**₹`r num(bootstrap_buyer$lower, 0)`–₹`r num(bootstrap_buyer$upper, 0)`**. The ordinary and

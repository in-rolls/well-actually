...
The [precision audit](../output/precision_ladder.csv) finds no implementation error in the original village-clustered regression standard errors. For the full buyer gap, the published method gives a 95% interval of ₹146–₹1,379. CR2 covariance with contrast-specific Satterthwaite inference widens it to ₹55–₹1,471 (about 33 degrees of freedom). A separate 9,999-draw null-imposed Webb wild village bootstrap gives ₹106–₹1,497; its p-value is 0.0173, versus 0.0219 under the WCR31 variant. WCR31 records two pseudoinverse warnings from district indicators losing support on village deletion; the [deletion audit](../output/precision_deletion_rank.csv) verifies that the focal contrasts remain estimable. The positive association survives these checks, but its magnitude remains uncertain. None supplies a confidence interval for the paper's unreconstructed percentage.

The original buyer comparison has observations in 44 upper-dominated and 42 lower-dominated villages. This is broader support than the later IHDS comparison; IHDS's roughly seven degrees of freedom should not be attributed to Anderson. Uneven information still matters: the five largest squared cluster scores account for 72% of the estimated CR1 variance of the original buyer contrast. That residual-dependent diagnostic describes concentration in the variance estimate, not the share of the effect explained by five villages or an effective village count. The [saved contributions](../output/precision_cluster_scores.csv) and [cell counts](../output/precision_cells.csv) make the calculation inspectable. The bootstrap and CR2 checks address sampling uncertainty; they cannot establish that caste classification is accurate or that village dominance is unconfounded.
...
Anderson used existing data from the 1997–98 World Bank survey of Uttar Pradesh and Bihar; she did not collect the survey herself. The questionnaire’s measurement limits and the paper’s subsequent variable construction are separate issues. The paper says the original survey sampled 2,250 households in 120 villages across 25 districts (p. 245). The downloaded survey metadata describes four strata: the 30 villages previously visited in the qualitative study were retained with 30 sampled households each; another 90 villages were selected through district and village sampling proportional to population, with 15 households each. These original sampling groups must not be equated with the 90 villages retained in Anderson’s regression without the roster crosswalk. The paper’s short description does not give those details. Random sampling of villages is not random assignment of caste dominance.
...
The [completed later-survey comparison](ihds-replication.md) does not reproduce a positive buyer gap in its primary crop-value-per-owned-acre model. The relative interval remains wide enough to include a 45% advantage, and using cultivated crop-season acreage changes the direction on the same households. These are different outcomes from Anderson’s cash sales. The finding strengthens the case for decomposing land use, leasing and cropping intensity before interpreting a sales/owned-area difference as physical productivity. The IHDS roster also reveals who participated in collective village reports, but it cannot measure respondent agreement.
